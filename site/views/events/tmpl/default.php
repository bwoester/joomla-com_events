...
<dl class="article-info">
  ...
  <?php if ($eventsModel->showLinkICalHelpArticle()): ?>
  <dd class="icalendar-help">
    Was ist iCalendar? <a href="<?php echo $eventsModel->getLinkICalHelpArticle(); ?>">iCalendar Hilfe</a>
  </dd>
  <?php endif; ?>
  <dd class="icalendar-link">
    iCalendar Link: <a href="<?php echo $eventsModel->getLinkICal(); ?>">iCalendar</a>
  </dd>
 </dl>
...
<?php foreach ($eventsModel->getEventsForCurrentPeriod() as $eventItem): ?>
  <tr>
    ...
    <td><?php
      ...
      echo $eventItem->title;
    
      if ($eventItem->cancelled)
      {
        echo '<span class="label label-cancelled" title="'.JText::_('COM_EVENTS_EVENT_CANCELLED_DESC').'">';
        echo JText::_('COM_EVENTS_EVENT_CANCELLED_LABEL');
        echo '</span>';
      }
